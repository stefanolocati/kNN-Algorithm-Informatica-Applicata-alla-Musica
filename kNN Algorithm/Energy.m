...
valPop = extractEnergy([file,'.wav']);
testPop = valPop(:,floor(length(valPop)/2)+1:end);
testlabelPop = repmat(1,1,length(testPop));    
    
valBlues = extractEnergy([file2,'.wav']);
testBlues =  valBlues(:,floor(length(valBlues)/2)+1:end);
testlabelBlues = repmat(2,1,length(testBlues));

valRock = extractEnergy([file3,'.wav']);
testRock = valRock(:,floor(length(valRock)/2)+1:end);
